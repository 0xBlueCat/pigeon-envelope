...
export const envelopeContractAddress =
...
export async function newCategory(
  ...
  const tx = await envelopCtr
    .connect(wallet)
    .newCategory(operator, categoryName, baseUri);
  ...
  console.log("newCategoryEvt:", JSON.stringify(evt.args, undefined, 2));
...
export async function getCategory(categoryId: string): Promise<{
  ...
  const category = await envelopCtr.getCategory(categoryId);
  return {
    categoryId: category.categoryId,
    operator: category.operator,
    baseURI: category.baseURI,
    categoryName: category.categoryName,
  };
}

export async function transferCategoryOperator(
  categoryId: string,
  newOperator: string
): Promise<void> {
  const tx = await envelopCtr
    .connect(walletOperator)
    .transferCategoryOperator(categoryId, newOperator);
  console.log("openTxHash:", tx.hash);
  tx.wait();
  const rcp = await provider.getTransactionReceipt(tx.hash);
  const evt = await envelopIface.parseLog(rcp.logs[0]);
  console.log(
    "transferCategoryOperatorEvt:",
...
export async function open(tokenId: string | number): Promise<void> {
  ...
  console.log("openEvt:", JSON.stringify(evt.args, undefined, 2));
}
...
export async function airdropEnvelopeNft(
  categoryId: string,
  addresses: string[]
): Promise<string> {
